...
class DoubanSync(_PluginBase):
    ...
    def parse_users(self) -> List[Tuple[str, Optional[str]]]:
        """
        解析用户列表
        """
        user_list = []
        for line in self._users.splitlines():
            parts = line.split(",")
            user_id = parts[0].strip()
            remark = parts[1].strip() if len(parts) > 1 else None
            if user_id:
                user_list.append((user_id, remark))
        return user_list
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'label': '用户列表',
                                            'rows': 5,
                                            'placeholder': '支持2种填写格式\n'
                                                            '第1种：豆瓣用户ID，每个一行\n'
                                                            '第2种：豆瓣用户ID,备注名，每组一行，豆瓣用户ID与备注名用英文逗号分隔'
                                        }
                                    }
                                ]
                            }
    ...
    def sync(self):
        ...
        user_list = self.parse_users()

        for user_id, remark in user_list:
            # 同步每个用户的豆瓣数据
            if not user_id:
                continue
            ...
            if not results:
                ...
                logger.info(f"获取到用户 {user_info} 豆瓣RSS数据：{len(results)}")
            # 解析数据
            ...
            logger.info(f"用户 {user_info} 豆瓣想看同步完成")
        # 保存历史记录
